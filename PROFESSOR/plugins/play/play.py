...
import config
from PROFESSOR import Apple, Resso, SoundCloud, Spotify, Telegram, YouTube, app
...
@app.on_callback_query(filters.regex("PROFmousAdmin") & ~BANNED_USERS)
async def PROFmous_check(client, CallbackQuery):
    try:
        await CallbackQuery.answer(
            "» Revert back to users account:\n\nOpen your group settings.\n-> Administriat\n-> Click on your name\n-> Uncheck anonymous admin permission.",
            show_alert=True,
        )
    except:
        pass
...
@app.on_callback_query(filters.regex("PROFPlaylists") & ~BANNED_USERS)
@languageCB
